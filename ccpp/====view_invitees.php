...
$sel= "select invitees from invitations where 1 and event_id = '$event_id'" ;
$result=$prs_pageing->number_pageing($sel,20,10,'N','Y') or die(mysql_error());
$data = mysql_fetch_array($result[0]);
$invitees_array = explode(",",$data['invitees']);
			...
			<td align="left" valign="top">
				...
				<tr>
						...
						<td width="4%" class="arial_13_bold">No.</td>
								<td width="84%"   class="arial_13_bold">Email Address</td>
								<td width="12%"   class="arial_13_bold">Attendance</td>
						 ...
						 ?>	 
							 ...
							 <td height="49" class="photo"> <input type="hidden" name="pid<?=$count;?>" id="pid<?=$count;?>" value="<?=$get->id;?>" /><?=$count;?>.</td>
							  ...
							  <td class="photo"><strong><? echo stripslashes($invitee); ?></strong></td>
							  <td class="photo"><strong><? if(is_attendee_by_email($invitee,$event_id) == true){ echo "Confirmed"; }else{ echo "Has Not Confirmed"; } ?></strong></td>
							  <?php /*?><td  align="center"nowrap class="photo" >				 
				  <input name="button" type="button" class="send_form" onClick="window.location.href='add_sub_admin.php?id=<?php echo ($get->id); ?>&mode=edit'" value=" EDIT ">
                    <input name="button2" type="button" class="send_form" onClick="deleteconfirm('Are you sure you want to delete this <?=$pagetitle;?>?. \n','add_sub_admin.php?id=<?php echo($get->id); ?>&mode=delete');" value="DELETE">
                  </td><?php */?>
