...
if(isset($_REQUEST['btnDelete']))
{
	...
	for($i = 1;$i <= $count1;$i++)
	{
		...
		{
			$query = "DELETE FROM  blog_posts where id=".$_REQUEST[$pid];
		}
		
		hb_get_result($query);
		
	}
	location("manage_resources.php?msg=3");
}
...
if(isset($_REQUEST['update']))
	{
		...
		for($i = 1;$i <= $count1;$i++)
		{
			...
			$is_feature = "is_feature".$i;
		 	 ...
		 	 $query = "update blog_posts  set is_feature='".$isactivevalue."' where id=".$_REQUEST[$pid];
			hb_get_result($query);
			
		}
		location("manage_resources.php?msg=2");
	}
$LeftLinkSection = 4;
$count = 0;
$pagetitle="Blog Posts";
$sel= "select * from blog_posts where blog_title like '".$_GET["order"]."%' order by post_date desc" ;
				...
				<table width="95%" align="center" border="0" cellpadding="0" cellspacing="0">
					...
					<? if($_GET["msg"]) { ?>
							<tr>
								...
								elseif($_GET["msg"]==4)
									echo "Resource with this name is already Exist.";	
								elseif($_GET["msg"]==5)
									echo "This Resource is in use. You can not delete this Resource.";	
				  ...
				  <tr>
					<td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  ...
						  <td align="left" valign="middle" class="title_wrapper_middle">Manage Resources</td>
				...
				<tr>
						...
						<td width="43" class="arial_13_bold">No.</td>
							<?php /*?><td width="130"   class="arial_13_bold">Featured</td><?php */?>
								<td width="130"   class="arial_13_bold">Resource Title</td>
								<td width="281"   class="arial_13_bold">Assign to Category</td>
								<td width="84"   class="arial_13_bold">Date</td>
	  					<td width="405" class="arial_13_bold">Actions</td>
				</tr>
						  <? $count=0; 
						 ...
						 <?php /*?><td class="photo" align="center"><input <?=($get->is_feature==1)?"checked":"";?>  type="checkbox" name="is_feature<?=$count; ?>"></td><?php */?>
							  <td class="photo"> <strong> <? echo stripslashes($get->blog_title); ?></strong></td>
					<td class="photo">
							<? 
							$field_result2=hb_get_result("select * from blog_categories where id in (".$get->category_id.") order by category_name");				
							$field_value_list2="";
							while($field_row2=mysql_fetch_array($field_result2))
							{
								if($field_value_list2=="")
								{
									$field_value_list2=stripslashes($field_row2["category_name"]);
								}
								else
								{
									$field_value_list2 .= ",".stripslashes($field_row2["category_name"]);
							...
							?>
							  ...
							  <td class="photo"> <strong> <? echo date("m-d-Y",strtotime(stripslashes($get->post_date))); ?></strong></td><td  align="center"nowrap class="photo" >				 <input name="button" type="button" class="send_form_big" onClick="window.location.href='blog_detail.php?post_id=<?php echo ($get->id); ?>'" value="USER COMMENTS">&nbsp;
				  <input name="button" type="button" class="send_form" onClick="window.location.href='add_resource.php?id=<?php echo ($get->id); ?>&mode=edit'" value=" EDIT ">
                    <input name="button2" type="button" class="send_form" onClick="deleteconfirm('Are you sure you want to delete this resource? \n','add_resource.php?id=<?php echo($get->id); ?>&mode=delete');" value="DELETE">
								  ...
								  <td  valign="middle">
									<table width="98%" border="0" cellpadding="2" cellspacing="2" align="center">
										...
										<td align="center" width="60"><input type="submit" name="btnDelete" id="btnDelete" value="DELETE" class="red" onclick="return chkDelete();" /></td>
											<td align="center" width="75"><input type="button" name="button2" id="button2" value="ADD NEW" class="add_new" onclick="location.href='add_resource.php?mode=add'" /></td>
