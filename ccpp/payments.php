...
if($_REQUEST['submit1'])
{
	...
	if($event_title != '')
	{
		$sel.= " and etitle like '%$event_title%' ";
	}
	$org_uname = $_REQUEST['org_uname'];
	if($org_uname != '')
	{
		$sel.= " and organizer.fname like '%$org_uname%' ";
	}
	$org_email = $_REQUEST['org_email'];
	if($org_email != '')
	{
		$sel.= " and organizer.email like '%$org_email%' ";
	}
	$org_lname = $_REQUEST['org_lname'];
	if($org_lname != '')
	{
		$sel.= " and organizer.lname like '%$org_lname%' ";
	}
	/*$uusrname = $_REQUEST['uusrname'];
	if($uusrname != '')
	{
	...
	if($ord_st_dt != '' && $ord_st_dt != "--")
	{
		$sel.= " and cdate >= '$ord_st_dt' ";
	}
	if($ord_end_dt != '' && $ord_end_dt != "--")
	{
		$sel.= " and  cdate <= '$ord_end_dt' ";
	}
...
else 
{
	...
	$sel= "select * from attendee where ufname like '".$_GET["order"]."%' and funding = 'Yes' order by cdate desc";
}
//echo $sel;
		  ...
		  <tr>
			...
			<td align="left" valign="top">
				...
				<form name="Search_option" id="Search_option" action="" method="post">
					...
					<tr>
						<td style="color:#FF0000;"><strong>Search Options :</strong></td>
					</tr>
						...
						<tr>
							...
							<td width="27%"><strong> Organizer's Email Address</strong>&nbsp;
							  <input type="text" name="org_email" id="org_email" value="<?=$_REQUEST['org_email'];?>" /></td>
							  <td width="27%"><strong>Organizer's First Name :</strong> &nbsp;&nbsp;
						...
						<tr>
						<td>
							<table width="662" cellpadding="0" cellspacing="0">
							<tr>
							
							<?php /*?><td width="20%"><strong>User's Username  :</strong>&nbsp;&nbsp;
							<input type="text" name="uusrname" id="uusrname" value="<?=$_REQUEST['uusrname']?>" />
							</td><?php */?>
							<td width="44%"><strong>User's Last Name  :</strong> &nbsp;&nbsp;
							  <input type="text" name="ulname" value="<?=$_REQUEST['ulname']?>" id="ulname" /></td>
							<td width="56%"><strong> User's Email Address :</strong>&nbsp;
				...
				<tr>
					...
					<td class="photo">
							...
							?>
							   ...
							   <td class="photo"><strong> <? echo $get->how_mch; ?></strong></td>
