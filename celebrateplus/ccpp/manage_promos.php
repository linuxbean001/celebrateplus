...
if(isset($_REQUEST['btnDelete']))
{
	...
	for($i = 1;$i <= $count1;$i++)
	{
		...
		{
			$query = "DELETE FROM  content_promos where id=".$_REQUEST[$pid];
		}
...
if(isset($_REQUEST['update']))
	{
		...
		for($i = 1;$i <= $count1;$i++)
		{
			...
			$query = "update content_promos set display_order=".$disp1.",active=".$active_value." where id=".$_REQUEST[$pid];
			hb_get_result($query);
...
$LeftLinkSection = 4;
$count = 0;
$pagetitle="Content Promos";
$sel= "select * from content_promos where navigation like '".$_GET["order"]."%' order by navigation,display_order" ;
		  ...
		  <tr>
			...
			<td align="left" valign="top">
				<table width="95%" align="center" border="0" cellpadding="0" cellspacing="0">
					...
					<? if($_GET["msg"]) { ?>
							<tr>
								...
								<?
								if($_GET["msg"]==1)
									echo "Content Promos Added Successfully.";
								elseif($_GET["msg"]==2)
									echo "Content Promos Updated Successfully.";
								elseif($_GET["msg"]==3)
									echo "Content Promos Deleted Successfully.";
								elseif($_GET["msg"]==4)
									echo "Content Promos with this name is already Exist.";	
								elseif($_GET["msg"]==5)
									echo "This Content Promos is in use. You can not delete this Content Promos.";	
				  ...
				  <tr>
					<td align="left" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
						  ...
						  <td align="left" valign="middle" class="title_wrapper_middle">Manage Content Promos</td>
				...
				<tr>
						...
						<td width="43" class="arial_13_bold">No.</td>
							
								<td width="130"   class="arial_13_bold">Promo Title</td>
								<td width="281"   class="arial_13_bold">Parent Page</td>
								<td width="112"  align="center" class="arial_13_bold">Display Order</td>
	  					<td width="350" class="arial_13_bold" align="center">Actions</td>
				</tr>
						  <? $count=0; 
					...
					<td class="photo">
							<?
								if($na != "")
								{
								  $q = "select * from staticpage where id in(".$na.")"; 
									$r = hb_get_result($q) or die(mysql_error());
									while($result_parent=mysql_fetch_array($r))
									{
										$page_header=stripslashes($result_parent['page_header']);
										echo $page_header.",";
									}
								}
									?>
							  ...
							  <td align="center" class="photo"><input onKeyPress="return keycode();" size="3" type="textbox" id="disp<?=$count;?>" name="disp<?=$count;?>" value="<?=$get->display_order;?>" ></td>
							  <td  align="center"nowrap class="photo" >						  <input name="button" type="button" class="send_form" onClick="window.location.href='add_promos.php?id=<?php echo ($get->id); ?>&mode=edit'" value=" EDIT ">
                    <input name="button2" type="button" class="send_form" onClick="deleteconfirm('Are you sure you want to delete this content promo? \n','add_promos.php?id=<?php echo($get->id); ?>&mode=delete');" value="DELETE">
                ...
                <? } ?>			
				 ...
				 <input type="hidden" name="count" id="count" value="<?=$count;?>" />   
								  ...
								  <td  valign="middle">
									<table width="98%" border="0" cellpadding="2" cellspacing="2" align="center">
										...
										<td align="center" width="60"><input type="submit" name="btnDelete" id="btnDelete" value="DELETE" class="red" onclick="return chkDelete();" /></td>
											<td align="center" width="75"><input type="button" name="button2" id="button2" value="ADD NEW" class="add_new" onclick="location.href='add_promos.php?mode=add'" /></td>
											<td width="825" colspan="7" align="left" style="padding-left:3px;"><input class="add_new" type="submit" name="update" value="UPDATE" ></td>
						...
						<script>
							...
							function chkDelete()
							{
								return confirm("Are you sure you want to delete this content promo?");
							}
						  </script>
								</td>
							</tr>
